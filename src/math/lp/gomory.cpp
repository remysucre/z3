...
struct create_cut {
    ...
    bool number_is_too_big(mpq const& a) const {
        return numerator(a) > m_big_number;
    }
...
public:
    ...
    lia_move cut() {
        ...
        m_abs_max = 0;
        for (const auto & p : m_row)
            m_abs_max = std::max(m_abs_max, abs(p.coeff()));
        ...
        if (m_t.is_empty()) {
            return report_conflict_from_gomory_cut(); // todo: actually, report the conflict!
        }
        m_dep = nullptr;
        for (auto c : *m_ex) 
...
// start creating cuts        
        for (unsigned j : columns_for_cuts) {
            ...
            if (abs(cr.polarity) == 1)  // delay the update of the bounds for j in a polar case
                polar_vars.push_back( {j, cr.polarity,  cc.m_dep} ); 
           
            if (cc.m_found_big || !is_small_cut(lia.m_t)) {
